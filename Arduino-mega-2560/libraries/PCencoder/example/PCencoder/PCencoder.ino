...
const float MAX_SPEED       = 4800.0;       // 最大速度   0.140625s   1800*0.140625=252
const float ACCELERATION    = 35600.0;      // 加速度
const long TARGET_POSITION  = 1280;         // 转90°位置  360/1.8=200   200*32=6400   6400/5=1280  
const float L_first = 18.7f;         // 第一段：2000计数        5cm
const float _L      = 21.4f;        // 之后无限重复：1280计数   20cm
const float L_last  = 21.0f;         // 第一段：2000计数        5cm              200步时暂停    4-1=3   增加3cm
...
void loop() {
  ...
  if (Serial1.available() >= 6) {
    ...
    if ( buf[5] == 0x5B) {
      ...
      if (step%2==1){if (millis()>=T && pkt.flag ==1){step+=1;T = millis() + 850;}    // 每次step增加的间隔为500ms
...
bool updateSowAndSpeed(float v_turn,float v_re) {
  ...
  if (step <=13 && step % 2 == 1) {
    ...
    Time_out = false;
    v = 60;                   // 拐弯时速度
    if (step<14) {k1=1.2;k2=1.5;}    //     1.2  1.2
...
bool b_stepper_move( ) {
  ...
  static const unsigned long pause_duration = 500;  // 设置延迟时间为1000-525=0.475
  if (!b_start ) 
  {
    b_start=true;
    sp_p = 0;
  ...
  if (sp_p==2 && myStepperp.distanceToGo() == 0) 
  {
